...
const Places = ({ places }) => {
	return (
		<div className="mx-auto max-w-full relative top-5 max-h-full flex flex-col gap-50 p-8 lg:max-w-7xl">
			{places.map((place) => (
				<div
					key={place._id}
					className="flex items-center gap-5 top-[5svh] w-full lg:max-w-7xl  "
				>
					<div className="relative w-full flex items-center justify-center">
						<img
							src={place.photos[0]}
							className="h-100 aspect-video absolute left-0 top-0 object-cover rounded-2xl shadow-xl shadow-primary-200/80"
							alt="Foto da acomodação"
						/>
					</div>
					<CornerDownLeft size={50} className="relative top-10" />
