					...
					<motion.div className="mt-12 w-full lg:max-w-4xl md:max-w-fit md:mx-auto px-4 max-sm:mt-0 max-sm:mb-2">
						...
						{hasActiveSearch() && (
							<motion.div
								...
								className="flex flex-nowrap gap-2 w-fit max-md:w-full mx-4 max-md:flex-wrap max-md:grid-cols-2  mt-5  max-md:justify-start justify-center items-center"
							>
								{badges.map((badge) => (
									<div
										key={badge.id}
										className={`inline-flex items-center gap-2 border bg-white pl-4 shadow-2xl pr-3 py-2 justify-start	rounded-full text-sm ${mobile && "!text-primary-900 !bg-primary-100 !border-primary-300"}`}
									>
										{badge.isGuests ? (
